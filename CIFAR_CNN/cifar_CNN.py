...
def initialize_network():
    ...
    conv_w1 = tf.Variable(tf.truncated_normal([5, 5, 3, 32], stddev=5e-4), dtype=tf.float32)
    ...
    conv_w2 = tf.Variable(tf.truncated_normal([5, 5, 32, 64], stddev=5e-4), dtype=tf.float32)
    ...
    conv_w3 = tf.Variable(tf.truncated_normal([5, 5, 64, 64], stddev=5e-4), dtype=tf.float32)
    ...
    # 第一全连接层，384个隐节点
    full_w1 = tf.Variable(tf.truncated_normal([4*4*64, 384], stddev=5e-4), dtype=tf.float32)
    ...
    # 第二全链接层
    full_w2 = tf.Variable(tf.truncated_normal([384, 192], stddev=5e-4), dtype=tf.float32)
    ...
    full_w3 = tf.Variable(tf.truncated_normal([192, 10], stddev=5e-4), dtype=tf.float32)
...
def training(epchos):
    ...
    x, y, train, correct_rate, dropout, layers, merged = initialize_network()
    ...
    try:
        for epcho in range(epchos):
            ...
            # 一次检测迭代
            ac, layers_r, merged_r = sess.run([correct_rate, layers, merged], feed_dict={x: testing_x, y: testing_y, dropout: 1.0})
            print("epcho {1} accuracy: {0}".format(ac, str(epcho)))
            train_writer.add_summary(merged_r, epcho)
        print("training over...")
