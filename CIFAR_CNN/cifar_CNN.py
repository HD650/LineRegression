...
training_file = "./data/reorganized_batch1"
...
def training(epchos):
    ...
    # 初始化数据
    training_set = image_pro.open_pickled_data(training_file)
    ...
    try:
        for epcho in range(epchos):
            ...
            # 从总样本中提取一小batch
            training_x = training_set["data"][index * batch_size:index * batch_size + batch_size, :]
            training_y = training_set["labels"][index * batch_size:index * batch_size + batch_size, :]
            testing_x = testing_set["data"][index * batch_size:index * batch_size + batch_size, :]
            testing_y = testing_set["labels"][index * batch_size:index * batch_size + batch_size, :]
            # 一次训练迭代
            ...
            print(str(valid[0]))
            print("epcho {1} accuracy: {0}".format(valid[1], str(epcho)))
